...
import com.apiguave.tinderclonecompose.databinding.FragmentSettingsBinding
...
class SettingsDialogFragment : androidx.fragment.app.DialogFragment() {
    ...
    private fun openRenditionPicker(renditionPlace: Int) {
        ...
        val renditions = RenditionType.entries.map { it.name }.toTypedArray()
        builder.setItems(renditions) { _, which ->
            val renditionType = RenditionType.entries.find { it.ordinal == which }
